...
export default function RecentLoadLogsTable() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card text-card-foreground p-4 sm:p-5 transition hover:border-primary/40 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="relative flex-1 sm:w-48 lg:w-52">
            ...
            <input
              ...
              className="h-8.5 w-full rounded-lg border border-border bg-background pl-8.5 pr-7 text-xs text-foreground placeholder:text-muted-foreground outline-none transition focus:border-[#FFA500] focus:ring-1 focus:ring-[#FFA500]/25"
            />
            {params.search && (
              <button
                onClick={() => setSearch("")}
          ...
          <div className="relative" ref={columnDropdownRef}>
            ...
            >
              <Columns3 size={13} className={visibleColumnsCount < RECENT_LOGS_COLUMNS.length ? "text-[#FFA500]" : "text-muted-foreground"} />
              <span>Columns</span>
          ...
          <button
            ...
            className="flex items-center gap-1.5 h-8.5 rounded-lg border border-border bg-background px-3 text-xs font-medium text-foreground hover:bg-muted hover:text-[#FFA500] transition cursor-pointer"
          >
            <Download size={13} />
            <span>Export</span>
          </button>
          ...
          <button
            onClick={() => refresh()}
            disabled={loading}
            className="flex h-8.5 w-8.5 items-center justify-center rounded-lg border border-border bg-background text-foreground hover:bg-muted transition cursor-pointer disabled:opacity-50"
            title="Refresh Data"
          >
      ...
      <div className="overflow-x-auto relative min-h-[220px] custom-scrollbar">
        ...
        <table className="w-full text-left text-xs whitespace-nowrap">
          ...
          <tbody className="divide-y divide-border">
            {data.length === 0 ? (
              <tr>
                <td colSpan={visibleColumnsCount || 9} className="py-10 text-center text-muted-foreground">
                  <div className="flex flex-col items-center justify-center gap-1">
                    <Search size={20} className="text-muted-foreground" />
                    <p className="font-medium text-foreground">No records match the current filters</p>
                    <button
                      onClick={resetFilters}
              ...
              data.map((row) => (
                ...
                >
                  ...
                  {/* Route */}
                  {visibleColumns.route && (
                    <td className="py-3 px-3 text-muted-foreground max-w-[140px] truncate" title={row.route}>
                      {row.route}
                    </td>
                  )}
      ...
      {/* Pagination Footer */}
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-t border-border pt-3 text-xs text-muted-foreground">
